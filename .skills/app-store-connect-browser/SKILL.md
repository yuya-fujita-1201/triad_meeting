...
| 暗号化コンプライアンスの「保存」ボタンが見つからない | ダイアログ内のアクセシビリティツリーに表示されないことがある。`read_page`の`interactive`フィルタで名前なしリンク（ref_XXX）を探す。それが保存ボタンの可能性が高い |
| スクリーンショットアップロードができない | ブラウザの`<input type="file">`はmacOSネイティブダイアログを開くためVM操作不可。**App Store Connect API経由でアップロードする**（`.skills/app-store-connect-api/SKILL.md`参照） |

---

## API自動化への移行ガイド

ブラウザ操作には限界があるため、以下の操作はApp Store Connect APIでの実行を推奨する。
APIスキルの詳細は **`.skills/app-store-connect-api/SKILL.md`** を参照。

| 操作 | ブラウザ | API | 推奨 |
|---|---|---|---|
| スクリーンショットアップロード | ✗（ファイルピッカー制約） | ✓ | **API** |
| 暗号化コンプライアンス設定 | △（ボタン不安定） | ✓ | **API** |
| 審査提出 | ✓ | ✓ | どちらでも |
| ビルド管理 | ✓ | ✓ | どちらでも |
| サブスクリプション設定 | ✓（nativeInputValueSetter必要） | ✗ | ブラウザ |
| アプリ登録 | ✓ | ✗ | ブラウザ |

---

## ワークフロー5: 暗号化コンプライアンス設定（ブラウザ操作）

**場所:** App Store Connect → アプリ → TestFlight → ビルド

### 手順
1. ビルド一覧で「コンプライアンスがありません 管理」のリンクをクリック
2. ダイアログでラジオボタンを選択（例:「上記のアルゴリズムのどれでもない」）
3. **保存ボタンの見つけ方が重要:**
   - `find("保存")` ではページレベルの保存ボタンが見つかることがある（ダイアログ内ではない）
   - `read_page` の `interactive` フィルタでダイアログ内の要素一覧を取得
   - 「キャンセル」の近くにある名前なしリンク（ref_XXX）が保存ボタンである可能性が高い
   - JavaScriptで `document.querySelectorAll('button')` をテキスト検索する方法も有効
4. 保存ボタンをクリック

### 代替手段
API経由での設定が安定する。`.skills/app-store-connect-api/SKILL.md` のワークフロー2を参照。
...
### 新規提出
1. バージョンページで「審査に追加」ボタンをクリック
2. 提出詳細を確認して「App Reviewに提出」をクリック
...
- 「13インチのiPadディスプレイのスクリーンショットをアップロードする必要があります」
  → TARGETED_DEVICE_FAMILY が "1,2" の場合、iPadスクリーンショットが必須。APIでアップロード
- 暗号化コンプライアンスが未設定のビルドは提出できない
