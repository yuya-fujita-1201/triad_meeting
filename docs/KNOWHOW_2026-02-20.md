...
xcodebuild -exportArchive -archivePath build/Runner.xcarchive -exportOptionsPlist ExportOptions.plist -exportPath build/ipa
    ↓
xcrun altool --upload-app --type ios -f "build/ipa/三賢会議.ipa" --apiKey JLULX74QSP --apiIssuer e359cd97-...
```
...
### 認証情報の保管
- `.appstoreconnect/private_keys/AuthKey_JLULX74QSP.p8` にプロジェクト内保存
